...
class Workspace:
    def __init__(self, cfg, logging=True):
        ...
        train_iterable = VIPBuffer(task=self.cfg.task, datatype=self.cfg.datatype, num_workers=self.cfg.num_workers, doaug=self.cfg.doaug)
        val_iterable = VIPBuffer(task=self.cfg.task, datatype=self.cfg.datatype, num_workers=self.cfg.num_workers, doaug=0)
